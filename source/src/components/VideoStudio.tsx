...
function AvatarTab() {
  ...
  const [avatarQuery, setAvatarQuery] = useState("Julian Goldie");  // default — your clones first
  const [voiceQuery, setVoiceQuery] = useState("Julian Goldie");
      ...
      <PreviewCard title={job ? `Avatar video · ${job.status}` : "Preview"} accent={ACCENT}>
        {job ? (
          ...
          <EmptyState icon={<User size={28} style={{ color: ACCENT }} />} title="Pick an avatar + voice → script → Generate"
            hint="AI avatar renders typically take 30–120 seconds. Your existing Julian Goldie SEO clones are pre-filtered. Status polls every 5s." />
        )}
      </PreviewCard>
