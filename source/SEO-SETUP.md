...
You do **not** edit any code for this. It's all in one config file.

Open `~/.agentic-os/config.json` (create it if it doesn't exist — copy
`agentic-os.config.example.json` from the `source/` folder).

Find the `seoSites` field and list your 5 sites:

```json
"seoSites": [
  { "id": "site1", "name": "mydomain1.com", "url": "https://mydomain1.com", "path": "~/my-site-1" },
  { "id": "site2", "name": "mydomain2.com", "url": "https://mydomain2.com", "path": "~/my-site-2" },
  { "id": "site3", "name": "mydomain3.com", "url": "https://mydomain3.com", "path": "~/my-site-3" },
  { "id": "site4", "name": "mydomain4.com", "url": "https://mydomain4.com", "path": "~/my-site-4" },
  { "id": "site5", "name": "mydomain5.com", "url": "https://mydomain5.com", "path": "~/my-site-5" }
],
"seoSkillSite": "site1"
```
...
`path` is the folder on your laptop (`~/...` is fine — your home folder is filled in automatically).

By default, posts are written to `<path>/src/blog/posts` (the Eleventy layout). If
your site stores posts somewhere else, add a `"postsDir"` to that entry — e.g.
Astro: `"postsDir": "~/my-site-1/src/content/blog"`.

`seoSkillSite` is which of your sites holds the `blog-post.md` skill + your
transcripts (under its `.claude` folder). Defaults to the first site if you omit it.

Save the file. (Leave `seoSites` as `null` and the dashboard just uses its built-in
default funnel — nothing breaks if you skip this.)
...
Make sure the `path` for each site in `~/.agentic-os/config.json` actually exists on your laptop.
